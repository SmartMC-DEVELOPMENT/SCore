...
public class ModuleClassLoader extends URLClassLoader {
    ...
    public static void loadPluginJar(File file) {
        try {
            ...
            ModulePlugin plugin = (ModulePlugin) loadedClass.getDeclaredConstructors()[0].newInstance();
            plugin.onEnable();
            System.out.println("Loaded plugin jar " + file + " mainClass=" + loadedClass.getName());
        } catch (Exception e) {
    ...
    public static ModulePluginInfo readPluginInfo(Class<? extends ModulePlugin> mainClass) {
        return mainClass.getDeclaredAnnotation(ModulePluginInfo.class);
    }
